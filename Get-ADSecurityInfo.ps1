...
Remove-Item "$Dir\ADSecurityCheck\log\*" -Force -ErrorAction Continue

if (!(Test-Path "$Dir\ADSecurityCheck\log")) {New-Item "$Dir\ADSecurityCheck\log" -type Directory -Force }
...
$LastLoggedOnDate = $(Get-Date) - $(New-TimeSpan -days $params.UserLogonAge)  
$PasswordStaleDate = $(Get-Date) - $(New-TimeSpan -days $params.UserPasswordAge)
...
[array]$DomainUsers = Get-ADUser -Filter * -Property $ADLimitedProperties -Server $($domaininfo.PDCEmulator)
...
[array]$DomainPasswordPolicy = Get-ADDefaultDomainPasswordPolicy -Server $($domaininfo.PDCEmulator)
...
$ADRootDSE = get-adrootdse  -Server $($domaininfo.PDCEmulator)
...
[array]$KerberosDelegationObjects =  Get-ADObject -filter { (UserAccountControl -BAND 0x0080000) -AND (PrimaryGroupID -ne '516') -AND (PrimaryGroupID -ne '521') } -Server $($domaininfo.PDCEmulator) -prop Name,ObjectClass,PrimaryGroupID,UserAccountControl,ServicePrincipalName
...
$DomainKRBTGTAccount = Get-ADUser 'krbtgt' -Server $($domaininfo.PDCEmulator) -Properties 'msds-keyversionnumber',Created,PasswordLastSet
...
foreach($group in $ADPrivGroupArray){
    try
    {
    $GrpProps = Get-ADGroupMember -Identity $group -Recursive -Server $($domaininfo.PDCEmulator) -ErrorAction SilentlyContinue | select SamAccountName,distinguishedName
    $tempobj = ""
        $GrpProps | % {
